...
3. **Check the actual failure signal**, not a proxy:
   - cron `error` state in `~/.hermes/cron/jobs.json` (not just "did it run")
...
`~/.hermes/scripts/unified_health_watchdog.py` (stdlib-only) + `~/.hermes/scripts/unified_health_watchdog.sh` wrapper. Reads Telegram creds from `~/.hermes/.env`, dedupes via `~/.hermes/scripts/.unified_watchdog_state.json`, sends to `TELEGRAM_HOME_CHANNEL`. **Copy and extend this; do not rewrite from scratch.** Structure documented in `references/watchdog-pattern.md`.

## Common failure modes the watchdog catches + fixes
See `references/cron-failure-modes.md`:
- cron `script` resolves relative to `~/.hermes/scripts/` — repo scripts must be copied or wrapped.
...
- `lab-health` is infra reachability (M1/M2/M3/Ollama/GPU), a different layer; this skill is automation error-state.
