---
name: check-m2-vram
description: Check current VRAM usage and loaded models on M2 (<lab-host>). Use whenever the user asks about GPU state, VRAM headroom, whether ComfyUI can run, whether the <content-pipeline> pipeline is active, or which models are currently loaded on the inference server.
---
...
4. Whether the <content-pipeline> pipeline appears to be running (ComfyUI process visible in ollama ps output or high GPU util with no LLM loaded)
5. **Tips & Caveats**
