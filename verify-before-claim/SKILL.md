...
version: 2026-08-24
author: NZ1Labs (<operator> / Hermes)
setup_needed: true
setup_skipped: false
---

# Verify-Before-Claim (<operator>'s trust rule — now enforced at runtime)

<operator> has caught Hermes stating unverified "facts" repeatedly — asserting a
...
| `<your-model> is loaded/not loaded` | ollama_model | curl :11434/api/ps (M2+M3) | high |
| `ComfyUI is up/down` | process | curl :8188/queue | high |
| `training is dead/done/running` | training | SSH→ps aux | high |
| `<content-pipeline> not posting` | process | wmic + content_history.json | high |
...
```
Verdict: FAIL
Claims: ['M2 is free', '<your-model> is not loaded', 'ComfyUI is running']

  [✓ VERIFIED] M2 state → GPU active, Ollama: <your-model>
  [✗ FAILED]   <your-model> not loaded → actually loaded on M2
...
|---|---|---|---|
| Negation not inverted | `"<your-model> is NOT loaded"` marked FAIL when probe confirmed absent | Phase 3 used same predicate for positive and negated claims | Negated claims now use `any(not p.verified for p in relevant_probes)` |
